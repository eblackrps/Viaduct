...
- The root `compose.yaml` is the exception for local Docker: it binds the host port to `127.0.0.1`, starts the API on `0.0.0.0` inside the container, and enables `--local-runtime-container` so the browser does not need a key.
...
- For local Docker evaluation, use `docker compose up -d --build` from the repo root instead of hand-writing keys or service-account setup.
