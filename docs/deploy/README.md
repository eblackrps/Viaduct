...
- Unix socket-style listener addresses are not considered loopback-equivalent for the local operator bootstrap, same-origin checks, or direct-loopback trust decisions.
...
- Keep local-runtime operator bootstrap on a loopback TCP bind such as `127.0.0.1`.
...
- For remotely reachable packaged environments, prefer explicit admin, tenant, or service-account credentials instead of local-runtime bootstrap.
