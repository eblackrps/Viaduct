...
Viaduct `v3.1.0` standardizes the dashboard visual system around a small shared token set so every page reads like the same application.
...
Dark mode is intentionally not shipped in `v3.1.0`.
