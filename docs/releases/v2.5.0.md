...
- loopback enforcement assumes TCP listeners; unix-socket or UDP/QUIC listeners require separate policy
...
- PostgreSQL credential upgrades now materialize a durable unique index on `credential_hashes.credential_hash`, so historical duplicate tenant or service-account keys must be remediated before startup can complete the migration
