...
| Errors / failures | every handler and middleware path | Weak | HTTP status codes are already mostly sensible for auth, bad request, conflict, not found, and rate limiting. `X-Request-ID` already exists. | Error bodies are plain text, not structured. There are no machine-readable error codes, no field errors, and no body-level request ID. OpenAPI does not describe failures as stable contracts. | Introduce one stable JSON error envelope and apply it across middleware, handlers, and reports. |
