...
This guide documents the default assessment workflow Viaduct ships for migration assessment, planning, and supervised pilot execution.
...
1. Create an assessment from the prefilled lab defaults.
2. Run discovery to save snapshots.
...
Read-only users can inspect assessment state and export reports with viewer access, but only operator-level principals can mutate assessment state or start jobs.
