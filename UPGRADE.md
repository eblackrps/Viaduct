...
- The loopback local operator bootstrap path now requires a direct `127.0.0.1` browser request. If you place the runtime behind a same-host reverse proxy or use any non-loopback hostname, authenticate with a tenant or service-account key instead of expecting `Use local operator session` to appear.
