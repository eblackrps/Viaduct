...
These manifests provide a minimal in-cluster Viaduct API deployment for labs, demos, and controlled pilots.

## Included Assets
- `configmap.yaml`: mounts a starter `config.yaml`
- `secret.example.yaml`: example secret manifest for the admin API key
- `deployment.yaml`: single-replica API deployment with probes and container hardening
...
- use an ingress or reverse proxy only after you have explicit auth, TLS, and tenant-key handling in place
