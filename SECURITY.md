...
- Prefer service account keys for normal access. Reserve tenant keys for setup or intentional tenant-admin actions.
...
- Treat the local session path as a direct loopback-only flow. Protected tenant routes should not rely on ambient anonymous fallback in any shared or proxied environment.
- Keep `VIADUCT_ALLOW_UNAUTHENTICATED_REMOTE=false` outside disposable emergency access scenarios. `viaduct serve-api` defaults to loopback and should not be exposed remotely without explicit API credentials.
