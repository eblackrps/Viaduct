...
type GraphSimulationLink = GraphEdge &
	d3.SimulationLinkDatum<GraphSimulationNode>;
...
export function DependencyGraph({
	...
	const topWorkloads = useMemo(
		() =>
			visibleGraph.nodes
				...
				.map((node) => ({
					node,
					degree: (adjacency.get(node.id) ?? []).length,
				}))
				.sort(
					(left, right) =>
	...
	const graphSummaryLabel =
		visibleGraph.nodes.length === 0
			? "Dependency graph showing no visible nodes."
			: `Dependency graph showing ${visibleGraph.nodes.length} nodes and ${visibleGraph.edges.length} edges, including ${visibleGraph.nodes.filter((node) => node.type === "vm").length} workloads.`;

	useEffect(() => {
		if (
			selectedNodeId &&
			!visibleGraph.nodes.some((node) => node.id === selectedNodeId)
		) {
			setSelectedNodeId(null);
		}
	...
	useEffect(() => {
		...
		const simulationNodes: GraphSimulationNode[] = visibleGraph.nodes.map(
			(node) => ({ ...node }),
		);
		const simulationLinks: GraphSimulationLink[] = visibleGraph.edges.map(
			(edge) => ({ ...edge }),
		);
		...
		const nodes = canvas
			...
			.join("circle")
			.attr("r", (node) =>
				node.id === selectedNodeId ? 18 : node.type === "vm" ? 14 : 12,
			)
			.attr("fill", (node: GraphSimulationNode) => nodeColor(node))
			.attr("stroke", (node) =>
				node.id === selectedNodeId ? "#0f172a" : "#ffffff",
			)
			...
			.on("click", (_event: MouseEvent, node: GraphSimulationNode) =>
				...
				<div className="flex flex-col gap-4">
					<div className="flex flex-wrap items-center gap-2">
						...
						<StatusBadge tone="neutral">
							{visibleGraph.nodes.filter((node) => node.type === "vm").length}{" "}
							workloads
						</StatusBadge>
						<StatusBadge tone="neutral">
							{
								visibleGraph.edges.filter((edge) => edge.type === "storage")
									.length
							}{" "}
							storage links
						</StatusBadge>
						<StatusBadge tone="neutral">
							{
								visibleGraph.edges.filter((edge) => edge.type === "backup")
									.length
							}{" "}
							backup links
						</StatusBadge>
									...
									<input
										...
										onChange={(event) =>
											setFilters((current) => ({
												...
												nodeTypes: {
													...current.nodeTypes,
													[key as GraphNode["type"]]: event.target.checked,
												},
											}))
										}
									/>
								...
								{topWorkloads.map(({ node, degree }) => (
									...
									>
										...
										<p className="mt-1 text-xs text-slate-500">
											{degree} direct relationship(s)
										</p>
									</button>
								))}
								{topWorkloads.length === 0 && (
									<p className="text-sm text-slate-500">
										No workloads are visible in the current graph scope.
									</p>
								)}
							</div>
						</div>
					...
					<div className="mt-4 space-y-4 text-sm text-slate-600">
						<div className="rounded-2xl bg-slate-50 px-4 py-4">
							<div className="flex flex-wrap items-center gap-2">
								...
								<StatusBadge
									tone={selectedNode.type === "vm" ? "info" : "neutral"}
								>
									{selectedNode.type}
								</StatusBadge>
							...
							<p className="mt-2 text-slate-500">
								{relatedNodes.length} direct relationship(s) in the current
								filtered graph scope.
							</p>
						</div>
							...
							<div className="mt-2 space-y-2">
								{relatedNodes.length > 0 ? (
									relatedNodes.map((relation) => (
										...
										>
											<div className="flex items-center justify-between gap-3">
												...
												<StatusBadge tone={edgeTone(relation.edge.type)}>
													{relation.edge.type}
												</StatusBadge>
											</div>
											<p className="mt-2 text-slate-500">
									...
									<p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-500">
										No direct relationships are visible for this node with the
										current filters.
									</p>
								)}
							</div>
...
function buildAdjacency(graph: {
	nodes: GraphNode[];
	edges: GraphEdge[];
}): Map<string, Array<{ edge: GraphEdge; node: GraphNode }>> {
...
function matchesSearch(node: GraphNode, search: string): boolean {
	...
	return [
		...
		node.type,
		node.platform ?? "",
		...Object.entries(node.metadata ?? {}).flat(),
	]
		.join(" ")
		.toLowerCase()
		.includes(text);
...
function endpointPosition(
	endpoint: GraphSimulationEndpoint,
	axis: "x" | "y",
): number {
