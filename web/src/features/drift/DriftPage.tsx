...
export function DriftPage({ latestSnapshot, overviewLoading, refreshToken }: DriftPageProps) {
  const { drift: report, loading, errors } = useLifecycleSignals({
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        badges={[
          { label: `${report?.events.length ?? 0} events`, tone: (report?.events.length ?? 0) > 0 ? "warning" : "success" },
          { label: latestSnapshot ? "Baseline available" : "No baseline", tone: latestSnapshot ? "info" : "neutral" },
        ]}
      />
      ...
      {error && !showLoading && !report && latestSnapshot && <ErrorState title="Drift comparison unavailable" message={error} />}

      {(report || latestSnapshot) && !showLoading && (
        <>
          <SectionCard title="Baseline context" description="Current baseline metadata and comparison scope.">
            <div className="grid gap-3 md:grid-cols-4">
              <DriftContext label="Baseline" value={report?.baseline.id ?? latestSnapshot?.id ?? "Unavailable"} />
              <DriftContext
              ...
              />
              <DriftContext label="Modified VMs" value={String(report?.modified_vms ?? 0)} />
              <DriftContext label="Policy drift" value={String(report?.policy_drifts ?? 0)} />
            </div>
          </SectionCard>
