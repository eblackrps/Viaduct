...
export function LifecyclePage({ summary, latestSnapshot, overviewLoading, refreshToken }: LifecyclePageProps) {
  const { costs, remediation, simulation, loading, simulationLoading, errors, simulate } = useLifecycleSignals({
  ...
  const lifecycleError = [errors.costs, errors.remediation, errors.simulation].filter(Boolean).join(" ");
  const recommendationError = [errors.remediation, errors.simulation].filter(Boolean).join(" ");
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        badges={[
          { label: `${summary?.recommendation_count ?? remediation?.recommendations.length ?? 0} recommendations`, tone: "success" },
      ...
      {hasLifecycleData && (
        <>
            ...
            <div className="grid gap-3 md:grid-cols-2">
              <div className="rounded-2xl bg-slate-50 px-4 py-4">
                ...
                <p className="mt-2 text-sm text-slate-500">
                  Use the dedicated Policy page to inspect rule-level violations and enforcement posture on the current inventory.
                </p>
              </div>
              <div className="rounded-2xl bg-slate-50 px-4 py-4">
                <p className="font-semibold text-ink">Drift review</p>
                <p className="mt-2 text-sm text-slate-500">
                  Use the Drift page to confirm baseline changes and unexpected workload movement before scheduling execution.
                </p>
              </div>
            </div>
          </SectionCard>
          ...
          />
          {errors.costs && <p className="rounded-2xl bg-rose-50 px-4 py-3 text-sm text-rose-700">{errors.costs}</p>}
