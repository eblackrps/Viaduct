...
const PLATFORM_OPTIONS: Platform[] = ["vmware", "proxmox", "hyperv", "kvm", "nutanix"];
...
export function WorkspacePage() {
  ...
  const activeJob = jobHistory.find((job) => job.status === "queued" || job.status === "running") ?? null;
  const reportHistory = useMemo(
    () =>
      [...(state.selectedWorkspace?.reports ?? [])].sort(
        (left, right) => new Date(right.exported_at).getTime() - new Date(left.exported_at).getTime(),
      ),
  ...
  const workflowSteps = useMemo(
    () => buildWorkflowSteps(state.selectedWorkspace, rows.length, inventoryWorkspace.selectedIds.length),
    [inventoryWorkspace.selectedIds.length, rows.length, state.selectedWorkspace],
  );
  const nextAction = useMemo(
    () => recommendedWorkspaceAction(state.selectedWorkspace, rows.length, inventoryWorkspace.selectedIds.length),
    [inventoryWorkspace.selectedIds.length, rows.length, state.selectedWorkspace],
  );
  const createValidation = useMemo(() => validateCreateForm(createForm), [createForm]);
  const createErrors = Object.values(createValidation).filter((value): value is string => Boolean(value));
  ...
  async function handleCreateWorkspace() {
    if (!canCreateWorkspace) {
      setState((current) => ({
        ...current,
        actionError: {
          message: "Complete the required workspace intake fields before creating the workspace.",
  ...
  if (state.error && state.workspaces.length === 0) {
    return (
      <ErrorState
        ...
        actions={[
          <button key="retry" type="button" onClick={() => void refreshWorkspaces()} className="operator-button-danger">
            Retry
          </button>,
        ]}
      />
    ...
    <div className="space-y-6">
      <PageHeader
        ...
        badges={[
          ...
          { label: state.selectedWorkspace?.status ?? "draft", tone: workspaceStatusTone(state.selectedWorkspace?.status) },
        ...
        actions={
          <>
            ...
            <button type="button" onClick={() => setShowCreateForm((current) => !current)} className="operator-button-secondary">
              {showCreateForm ? "Hide intake" : "New workspace"}
            </button>
            <button
              type="button"
              onClick={() => void (selectedWorkspaceID ? refreshWorkspaceDetail(selectedWorkspaceID) : Promise.resolve())}
      ...
      <section className="grid gap-5 xl:grid-cols-[1.05fr_0.95fr]">
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            <Metric label="Workspace status" value={state.selectedWorkspace?.status ?? "draft"} tone={workspaceStatusTone(state.selectedWorkspace?.status)} />
            <Metric label="Selected workloads" value={String(inventoryWorkspace.selectedIds.length)} tone={inventoryWorkspace.selectedIds.length > 0 ? "accent" : "neutral"} />
            <Metric label="Snapshots" value={String(state.selectedWorkspace?.snapshots?.length ?? 0)} tone={(state.selectedWorkspace?.snapshots?.length ?? 0) > 0 ? "info" : "neutral"} />
            <Metric label="Reports" value={String(reportHistory.length)} tone={reportHistory.length > 0 ? "success" : "neutral"} />
            <Metric
              label="Readiness"
              value={state.selectedWorkspace?.readiness?.status ?? "pending"}
              tone={readinessTone(state.selectedWorkspace?.readiness?.status)}
            />
            <Metric label="Notes" value={String(noteHistory.length)} tone={noteHistory.length > 0 ? "info" : "neutral"} />
          </div>
          ...
          {activeJob ? (
            <div className="mt-4 rounded-[1.4rem] border border-amber-200 bg-amber-50 px-4 py-4 text-sm text-amber-900">
              ...
              <p className="mt-2 text-xs text-amber-800">
                Correlation ID {activeJob.correlation_id ?? "n/a"} · Requested {formatTimestamp(activeJob.requested_at)}
              </p>
            </div>
          ) : null}
          ...
          {state.selectedWorkspace?.saved_plan ? (
            <div className="mt-4 metric-surface">
              ...
              <p className="mt-2 text-sm font-semibold text-ink">{state.selectedWorkspace.saved_plan.spec_name}</p>
              <p className="mt-1 text-sm text-slate-600">
                {state.selectedWorkspace.saved_plan.workload_count} workload(s) · Generated {formatTimestamp(state.selectedWorkspace.saved_plan.generated_at)}
              </p>
            </div>
          ) : null}
        </SectionCard>
        ...
        >
          <div className="grid gap-4 lg:grid-cols-2">
            <WorkspaceSelectField
              ...
              value={settingsDraft.target_assumptions?.platform ?? state.selectedWorkspace?.target_assumptions?.platform ?? "proxmox"}
              options={PLATFORM_OPTIONS}
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, platform: value as Platform },
                }))
              }
            />
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, address: value },
                }))
              }
            />
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, default_host: value },
                }))
              }
            />
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, default_storage: value },
                }))
              }
            />
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, default_network: value },
                }))
              }
            />
            <WorkspaceField
            ...
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, approval_ticket: value },
                }))
              }
            />
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, window_start: value },
                }))
              }
            ...
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, window_end: value },
                }))
              }
            ...
            <WorkspaceField
              ...
              onChange={(value) =>
                setSettingsDraft((current) => ({
                  ...current,
                  target_assumptions: { ...current.target_assumptions, notes: value },
                }))
              }
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-3">
            <WorkspaceCheckbox
              ...
              onChange={(checked) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, verify_boot: checked },
                }))
              }
            />
            <WorkspaceCheckbox
              ...
              onChange={(checked) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, approval_required: checked },
                }))
              }
            />
            <WorkspaceCheckbox
              ...
              onChange={(checked) =>
                setSettingsDraft((current) => ({
                  ...current,
                  plan_settings: { ...current.plan_settings, dependency_aware: checked },
                }))
              }
            />
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <button type="button" onClick={() => void handleWorkspaceUpdate(settingsDraft, "settings")} className="operator-button">
              {actionLoading === "settings" ? "Saving..." : "Save workspace settings"}
            </button>
          </div>
        </SectionCard>
        ...
        <div className="space-y-5">
          ...
          >
            <div className="metric-surface">
              ...
              <p className="mt-2 text-sm font-semibold text-ink">{nextAction}</p>
            </div>
            <div className="mt-4 flex flex-wrap gap-2">
              <button type="button" onClick={() => void handleJob("discovery")} className="operator-button-secondary">
                {actionLoading === "discovery" ? "Running discovery..." : "Run discovery"}
              </button>
              <button type="button" onClick={() => void handleJob("graph")} className="operator-button-secondary">
                {actionLoading === "graph" ? "Building graph..." : "Build graph"}
              </button>
              <button type="button" onClick={() => void handleJob("simulation")} className="operator-button-secondary">
                {actionLoading === "simulation" ? "Running simulation..." : "Run simulation"}
              </button>
              <button type="button" onClick={() => void handleJob("plan")} className="operator-button-secondary">
                {actionLoading === "plan" ? "Saving plan..." : "Save plan"}
              </button>
              <button type="button" onClick={() => void handleExportReport()} className="operator-button-secondary">
          ...
          >
            {latestSource ? (
              <div className="grid gap-3">
                ...
                <HistoryRow label="Credential reference" value={latestSource.credential_ref || "Not set"} />
                <HistoryRow label="Latest discovery" value={latestSource.last_discovered_at ? formatTimestamp(latestSource.last_discovered_at) : "No discovery recorded"} />
                <HistoryRow label="Latest snapshot" value={latestSource.last_snapshot_id || "No snapshot recorded"} />
              </div>
            ) : (
              <EmptyState
      ...
      <section className="grid gap-5 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        >
          <div className="grid gap-3 md:grid-cols-2">
            <Metric label="Readiness status" value={state.selectedWorkspace?.readiness?.status ?? "pending"} tone={readinessTone(state.selectedWorkspace?.readiness?.status)} />
            <Metric
              label="Selected workloads"
              value={String(state.selectedWorkspace?.readiness?.selected_workload_count ?? inventoryWorkspace.selectedIds.length)}
              tone={inventoryWorkspace.selectedIds.length > 0 ? "accent" : "neutral"}
            />
            <Metric
              label="Policy issues"
              value={String(state.selectedWorkspace?.readiness?.policy_violation_count ?? 0)}
              tone={(state.selectedWorkspace?.readiness?.policy_violation_count ?? 0) > 0 ? "warning" : "success"}
            />
            <Metric
              label="Recommendations"
              value={String(state.selectedWorkspace?.readiness?.recommendation_count ?? 0)}
              tone={(state.selectedWorkspace?.readiness?.recommendation_count ?? 0) > 0 ? "info" : "neutral"}
            />
          </div>

          {(state.selectedWorkspace?.readiness?.blocking_issues?.length ?? 0) > 0 ? (
            <IssueList
          ...
          {(state.selectedWorkspace?.readiness?.warning_issues?.length ?? 0) > 0 ? (
            <IssueList
              title="Warnings"
          ...
          {reportHistory.length > 0 ? (
            <div className="mt-4 metric-surface">
              ...
              <p className="mt-2 text-sm font-semibold text-ink">{reportHistory[0].file_name}</p>
              <p className="mt-1 text-sm text-slate-600">Exported {formatTimestamp(reportHistory[0].exported_at)}</p>
            </div>
          ) : null}
        </SectionCard>
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div className="space-y-2">
                      <div className="flex flex-wrap items-center gap-2">
                        <StatusBadge tone={workspaceJobTone(job.status)}>{job.type}</StatusBadge>
                        <StatusBadge tone={workspaceJobTone(job.status)}>{job.status}</StatusBadge>
                      ...
                      <p className="text-xs text-slate-500">
                        ...
                        {job.requested_by ? ` by ${job.requested_by}` : ""} · Correlation ID {job.correlation_id ?? "n/a"}
                      </p>
                    ...
                    <div className="flex flex-wrap gap-2">
                      <button
                        type="button"
                        onClick={() => void refreshWorkspaceDetail(state.selectedWorkspace?.id ?? "")}
        ...
        <section className="grid gap-5 xl:grid-cols-[1.35fr_0.85fr]">
          <InventoryTable
            ...
            actions={
              <button
                type="button"
                onClick={() => void handleWorkspaceUpdate({ selected_workload_ids: inventoryWorkspace.selectedIds }, "selection")}
          ...
          <WorkloadDetailPanel
            ...
            onPreparePlan={(row) => inventoryWorkspace.replaceSelection([row.id])}
          />
        </section>
      )}
      ...
      <section className="grid gap-5 xl:grid-cols-[1.1fr_0.9fr]">
        ...
        >
                ...
                void handleWorkspaceUpdate(
                  {
                    notes: [
                      ...(state.selectedWorkspace?.notes ?? []),
                      {
                        id: "",
                        kind: "operator",
                        author: state.selectedWorkspace?.tenant_id ?? "operator",
                        body: noteDraft,
                        created_at: "",
                      } as WorkspaceNote,
                    ],
          ...
          <div className="mt-5 space-y-4">
            <ArtifactBlock title="Recent notes" emptyMessage="No operator notes saved yet.">
              {noteHistory.slice(0, 4).map((note) => (
                <div key={`${note.author}-${note.created_at}-${note.body}`} className="metric-surface">
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge tone={note.kind === "operator" ? "accent" : "neutral"}>{note.kind}</StatusBadge>
                    <StatusBadge tone="neutral">{note.author}</StatusBadge>
                  </div>
                  <p className="mt-3 text-sm text-slate-700">{note.body}</p>
                  <p className="mt-2 text-xs text-slate-500">{formatTimestamp(note.created_at)}</p>
                </div>
              ))}
            </ArtifactBlock>

            <ArtifactBlock title="Export history" emptyMessage="No reports exported yet.">
              {reportHistory.slice(0, 4).map((report) => (
                <div key={report.id} className="metric-surface">
                  <p className="text-sm font-semibold text-ink">{report.file_name}</p>
                  <p className="mt-1 text-sm text-slate-600">
                    {report.name} · {report.format} · {formatTimestamp(report.exported_at)}
                  </p>
                  <p className="mt-2 text-xs text-slate-500">Correlation ID {report.correlation_id ?? "n/a"}</p>
                </div>
              ))}
            </ArtifactBlock>

            <ArtifactBlock title="Approvals" emptyMessage="No approval records attached yet.">
              {(state.selectedWorkspace?.approvals ?? []).slice(0, 4).map((approval) => (
                <div key={approval.id} className="metric-surface">
                  <div className="flex flex-wrap items-center gap-2">
                    <StatusBadge tone={approval.status === "approved" ? "success" : approval.status === "rejected" ? "danger" : "warning"}>
                      {approval.status}
                    </StatusBadge>
                    <StatusBadge tone="neutral">{approval.stage}</StatusBadge>
                  </div>
                  <p className="mt-3 text-sm text-slate-700">
                    {approval.approved_by ? `Handled by ${approval.approved_by}` : "Pending operator approval"}
                    {approval.ticket ? ` · Ticket ${approval.ticket}` : ""}
                  </p>
                  {approval.notes ? <p className="mt-2 text-sm text-slate-600">{approval.notes}</p> : null}
                  <p className="mt-2 text-xs text-slate-500">{formatTimestamp(approval.created_at)}</p>
                </div>
              ))}
            </ArtifactBlock>
          </div>
    ...
    >
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <WorkspaceSelectField
          ...
          options={PLATFORM_OPTIONS}
          onChange={(value) => onChange({ ...createForm, sourcePlatform: value as Platform })}
        />
        <WorkspaceField
          label="Source address"
          value={createForm.sourceAddress}
          onChange={(value) => onChange({ ...createForm, sourceAddress: value })}
          error={validation.sourceAddress}
        />
        <WorkspaceField
          label="Credential reference"
          value={createForm.sourceCredentialRef}
          onChange={(value) => onChange({ ...createForm, sourceCredentialRef: value })}
        ...
        <WorkspaceSelectField
          ...
          options={PLATFORM_OPTIONS}
          onChange={(value) => onChange({ ...createForm, targetPlatform: value as Platform })}
        />
        <WorkspaceField
          label="Target address"
          value={createForm.targetAddress}
          onChange={(value) => onChange({ ...createForm, targetAddress: value })}
        ...
        <WorkspaceField
          ...
          onChange={(value) => onChange({ ...createForm, defaultStorage: value })}
          error={validation.defaultStorage}
        />
        <WorkspaceField
          label="Default network"
          value={createForm.defaultNetwork}
          onChange={(value) => onChange({ ...createForm, defaultNetwork: value })}
          error={validation.defaultNetwork}
        />
      </div>
      <div className="mt-4 flex flex-wrap gap-2">
        <button type="button" onClick={onSubmit} disabled={creating} className="operator-button">
...
function WorkspaceField({
  ...
  return (
    <label className="block">
      ...
      <input value={value} onChange={(event) => onChange(event.target.value)} className="operator-input mt-2" />
...
function WorkspaceSelectField({
  ...
  return (
    <label className="block">
      <span className="operator-kicker">{label}</span>
      <select value={value} onChange={(event) => onChange(event.target.value)} className="operator-select mt-2">
        {options.map((option) => (
  ...
  return (
    <label className="metric-surface flex items-start gap-3">
      ...
      <span>
        ...
        <span className="mt-1 block text-sm text-slate-600">Persist this plan behavior in the workspace so future runs and exports use the same intent.</span>
      </span>
    </label>
  );
}
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            ...
            <StatusBadge tone={workflowStepTone(step.status)}>{step.status === "current" ? "next" : step.status}</StatusBadge>
          </div>
  ...
  return (
    <div>
      ...
      <div className="mt-3 space-y-3">
        {hasChildren ? children : <div className="metric-surface text-sm text-slate-600">{emptyMessage}</div>}
      </div>
    </div>
  );
}

function IssueList({ title, items, tone }: { title: string; items: string[]; tone: StatusTone }) {
  return (
    <div className="mt-4">
  ...
  return [
    {
      ...
      summary: "Capture the source connection, credential reference, target platform, and initial plan defaults.",
      evidence: workspace ? `Workspace ${workspace.name} is persisted.` : "No workspace exists yet.",
      status: stepStatus(states[0], currentIndex, 0),
    },
    {
      id: "discovery",
      label: "Run discovery",
      summary: "Persist a snapshot baseline so later graph, readiness, and report views reflect the same source evidence.",
      evidence:
        (workspace?.snapshots?.length ?? 0) > 0
    ...
    },
    {
      ...
      summary: "Review the discovered workloads, dependency context, and the selection you want to carry into planning.",
      evidence:
        workloadCount > 0
    ...
    },
    {
      ...
      summary: "Derive readiness, policy issues, and recommendation signals against the same persisted workspace.",
      evidence: workspace?.readiness ? `Readiness is ${workspace.readiness.status}.` : "Simulation has not been run yet.",
      status: stepStatus(states[3], currentIndex, 3),
    },
    {
      id: "plan",
      label: "Save plan",
      summary: "Persist a dry-run migration plan so operator review, approval, and export can use the same state.",
      evidence: workspace?.saved_plan ? `Saved plan ${workspace.saved_plan.spec_name}.` : "No plan saved yet.",
      status: stepStatus(states[4], currentIndex, 4),
    },
    {
      id: "report",
      label: "Export report",
      summary: "Keep the handoff artifact attached to the workspace so review and pilot follow-up stay traceable.",
      evidence:
        (workspace?.reports?.length ?? 0) > 0
...
function stepStatus(completed: boolean, currentIndex: number, index: number): WorkflowStep["status"] {
  if (completed) {
    return "complete";
  }
...
function validateCreateForm(createForm: CreateFormState): Record<keyof CreateFormState, string | null> {
  return {
    name: createForm.name.trim() === "" ? "Workspace name is required." : null,
    description: null,
    sourceName: createForm.sourceName.trim() === "" ? "Source name is required." : null,
    sourcePlatform: null,
    sourceAddress: createForm.sourceAddress.trim() === "" ? "Source address is required." : null,
    sourceCredentialRef: createForm.sourceCredentialRef.trim() === "" ? "Credential reference is required." : null,
    targetPlatform: null,
    targetAddress: createForm.targetAddress.trim() === "" ? "Target address is required." : null,
    defaultHost: createForm.defaultHost.trim() === "" ? "Default host is required." : null,
    defaultStorage: createForm.defaultStorage.trim() === "" ? "Default storage is required." : null,
    defaultNetwork: createForm.defaultNetwork.trim() === "" ? "Default network is required." : null,
  };
}
...
function readinessTone(status?: PilotWorkspace["readiness"] extends infer T
