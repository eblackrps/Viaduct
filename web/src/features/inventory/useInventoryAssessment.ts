...
import type { DependencyGraph, DiscoveryResult, SnapshotMeta } from "../../types";
...
export function useInventoryAssessment({
  ...
  const { policies, remediation, loading: lifecycleLoading, errors: lifecycleErrors } = useLifecycleSignals({
  ...
  const sources = useMemo<InventoryAssessmentSourceState>(
    () => ({
      ...
      policies: !lifecycleLoading && !lifecycleErrors.policies && policies !== null,
      remediation: !lifecycleLoading && !lifecycleErrors.remediation && remediation !== null,
    }),
    [graphError, graphLoading, hasInventory, lifecycleErrors.policies, lifecycleErrors.remediation, lifecycleLoading, policies, remediation],
  );
  ...
  const rows = useMemo(
    ...
    [graph, inventory, latestSnapshot?.discovered_at, policies, remediation, sources],
  );
  ...
  return {
    ...
    errors,
    error: [errors.graph, errors.policies, errors.remediation].filter(Boolean).join(" ") || null,
    topSignals,
    sources,
  };
}
