...
export function buildInventoryAssessmentRows(
	...
	return vms.map((vm) => {
		...
		const policyViolations = lookupInventoryItems(
			policyIndex,
			vm,
			getPolicyViolationKey,
		);
		const recommendations = lookupInventoryItems(
			recommendationIndex,
			vm,
		...
		const enforceViolations = policyViolations.filter(
			(item) => item.severity === "enforce",
		);
		const warningViolations = policyViolations.filter(
			(item) => item.severity === "warn",
		);
		const snapshotCount = vm.snapshots?.length ?? 0;
		const missingStorageMetadata = vm.disks.filter(
			(disk) => disk.storage_backend.trim() === "",
		).length;
		const powerUnknown = vm.power_state === "unknown";
		const noBackupRelationship =
			dependencySummary.graphResolved &&
			dependencyIndex.hasBackupSignals &&
			dependencySummary.backups.length === 0;
		const networkGap =
			dependencySummary.graphResolved &&
			vm.nics.length > 0 &&
			dependencySummary.networks.length === 0;
		...
		if (enforceViolations.length > 0) {
			...
			riskReasons.push(
				`${enforceViolations.length} enforce-level policy violation(s)`,
			);
		}
		if (warningViolations.length > 0) {
			...
			riskReasons.push(
				`${warningViolations.length} warning-level policy issue(s)`,
			);
		}
		if (recommendations.length > 0) {
			...
			riskReasons.push(
				`${recommendations.length} remediation recommendation(s)`,
			);
		}
		if (snapshotCount > 0) {
		...
		if (missingStorageMetadata > 0) {
			...
			riskReasons.push(
				`Storage backend missing on ${missingStorageMetadata} disk(s)`,
			);
		}
		if (powerUnknown) {
		...
		if (noBackupRelationship) {
			...
			riskScore += 1;
			riskReasons.push(
				"No backup relationship is exposed in the current graph",
			);
		}
		if (networkGap) {
			...
			riskScore += 1;
			riskReasons.push(
				"Network dependencies could not be resolved from the graph",
			);
		}
		if (staleDiscovery) {
		...
		const connectedNetworks = Array.from(
			new Set(vm.nics.map((nic) => nic.network.trim()).filter(Boolean)),
		);
		...
		const storageTotalMB = vm.disks.reduce(
			(total, disk) => total + disk.size_mb,
			0,
		);
		...
		switch (sortKey) {
			...
			case "recency":
				comparison = compareNumbers(
					timestampValue(left.discoveredAt),
					timestampValue(right.discoveredAt),
				);
				break;
			case "dependencies":
...
export function summarizeAssessmentSignals(
	rows: InventoryAssessmentRow[],
...
): Map<string, RemediationRecommendation[]> {
...
function buildDependencyIndex(graph: DependencyGraph | null): {
	...
	return {
		...
		hasBackupSignals: (graph?.nodes ?? []).some(
			(node) => node.type === "backup-job",
		),
		nodeById,
		relationsByNodeId,
	};
...
function resolveDependenciesForVM(
	...
	const candidateNodeIDs = [`vm:${vm.id}`, `vm:${vm.name}`].filter(
		(value) => value !== "vm:",
	);
	const nodeID =
		candidateNodeIDs.find((value) => index.nodeById.has(value)) ??
		Array.from(index.nodeById.values()).find(
			(node) =>
				node.type === "vm" &&
				node.label.trim().toLowerCase() === vm.name.trim().toLowerCase(),
		)?.id;
	const relations = nodeID
		? dedupeRelations(index.relationsByNodeId.get(nodeID) ?? [])
		: [];
	const networks = relations
		.filter((relation) => relation.node.type === "network")
		.map((relation) => relation.node);
	const datastores = relations
		.filter((relation) => relation.node.type === "datastore")
		.map((relation) => relation.node);
	const backups = relations
		.filter((relation) => relation.node.type === "backup-job")
		.map((relation) => relation.node);
	...
	return {
		...
		counts: {
			network: relations.filter((relation) => relation.edgeType === "network")
				.length,
			storage: relations.filter((relation) => relation.edgeType === "storage")
				.length,
			backup: relations.filter((relation) => relation.edgeType === "backup")
				.length,
		},
...
function lookupInventoryItems<T>(
	index: Map<string, T[]>,
	vm: VirtualMachine,
...
function buildSearchDocument(
	...
	return [
		...
		vm.nics
			.map((nic) => {
				const ipAddresses = Array.isArray(nic.ip_addresses)
					? nic.ip_addresses
					: [];
				return `${nic.name} ${nic.network} ${ipAddresses.join(" ")}`;
			})
			.join(" "),
		...
		dependencies.relations
			.map(
				(relation) =>
					`${relation.edgeType} ${relation.node.label} ${relation.label}`,
			)
			.join(" "),
		policyViolations
			.map(
				(violation) =>
					`${violation.policy.name} ${violation.rule.field} ${violation.remediation ?? ""}`,
			)
			.join(" "),
		recommendations
			.map(
				(recommendation) =>
					`${recommendation.summary} ${recommendation.action} ${recommendation.type}`,
			)
			.join(" "),
	]
		.join(" ")
