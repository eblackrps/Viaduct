...
export function useInventoryWorkspace(rows: InventoryAssessmentRow[], initialSelectedIDs: string[] = []): InventoryWorkspaceState {
  ...
  const filteredRows = useMemo(() => {
    const filtered = filterInventoryRows(rows, { ...filters, scope: "all", search: deferredSearch });
    const scopedRows = filters.scope === "selected"
  ...
  const availablePlatforms = useMemo<Platform[]>(
    () => [...new Set(rows.map((row) => row.vm.platform))].sort((left, right) => left.localeCompare(right)),
    [rows],
  );
  ...
  const summary = useMemo(() => summarizeInventoryRows(rows, selectedRows.length), [rows, selectedRows.length]);
  ...
  function changeSort(nextKey: InventorySortKey) {
    ...
    setSortKey(nextKey);
    setSortDirection(nextKey === "name" || nextKey === "platform" ? "asc" : "desc");
  }
