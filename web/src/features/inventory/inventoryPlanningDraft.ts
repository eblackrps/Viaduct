...
export function loadInventoryPlanningDraft(): InventoryPlanningDraft | null {
	...
	try {
		...
		if (
			!draft ||
			!Array.isArray(draft.workloads) ||
			typeof draft.sourcePlatform !== "string"
		) {
...
export function saveInventoryPlanningDraft(
	draft: InventoryPlanningDraft,
): void {
	if (typeof window === "undefined") {
		return;
	}
