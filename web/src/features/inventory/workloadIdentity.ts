...
type WorkloadIdentityInput = Pick<VirtualMachine, "platform" | "source_ref" | "id" | "name">;
...
export function getVirtualMachineLookupKeys(vm: Partial<WorkloadIdentityInput>): string[] {
  const platform = normalizeToken(vm.platform);
...
export function getPlatformScopeLabel(platform: Platform | "" | null | undefined): string {
  return normalizeToken(platform) || "mixed";
}

function normalizeToken(value: string | Platform | null | undefined): string {
