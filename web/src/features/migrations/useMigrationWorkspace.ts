...
export function useMigrationWorkspace({
	...
	const [importedDraft, setImportedDraft] =
		useState<InventoryPlanningDraft | null>(planningDraft ?? null);
	...
	const selectedWorkloads = useMemo(
		() =>
			(inventory?.vms ?? []).filter((vm) =>
				selectedWorkloadKeys.includes(getVirtualMachineIdentity(vm)),
			),
	...
	const preflightStale = Boolean(
		preflight && preflightSpecKey !== specFingerprint,
	);
	const planStale = Boolean(
		migrationState && plannedSpecKey && plannedSpecKey !== specFingerprint,
	);
	const validationError = validateMigrationDraft(
	...
	const executionBlockers = [
		...(planStale
			? [
					"Draft inputs changed after this plan was saved. Save a new plan before executing.",
				]
			: []),
		...(migrationState?.errors ?? []).map(
			(item) => `Saved plan issue: ${item}`,
		),
		...(migrationState?.pending_approval
			? ["This saved plan is waiting on approval before execution can proceed."]
			: []),
		...(!migrationState?.pending_approval &&
		approvalRequired &&
		approvedBy.trim() === "" &&
		migrationState?.phase === "plan"
			? ["Execution approval is still required."]
			: []),
		...(windowState.kind === "not-started"
			? [`Execution window opens at ${windowState.label}.`]
			: []),
		...(windowState.kind === "closed"
			? [`Execution window closed at ${windowState.label}.`]
			: []),
	];
	const executionAdvisories = [
		...(preflightStale
			? [
					"Validation results are stale for the current draft. Rerun preflight before treating this plan as ready.",
				]
			: []),
	...
	const isPolling = Boolean(
		migrationState &&
		executionRequested &&
		isMigrationExecutionActivePhase(migrationState.phase),
	);
	...
	useEffect(() => {
		...
		setInventory({
			...
			platform: planningDraft.sourcePlatform,
			vms: planningDraft.workloads,
		});
	...
	const refreshSavedState = useCallback(
		async (options?: { silent?: boolean }) => {
			...
			try {
				...
				if (
					requestSequence === stateRefreshSequenceRef.current &&
					!isAbortError(reason)
				) {
	...
	}, [isPolling, migrationID, migrationState, refreshSavedState]);
		...
		try {
			...
			if (!importedDraft) {
				const availableKeys = new Set(
					result.inventory.vms.map((vm) => getVirtualMachineIdentity(vm)),
				);
				setSelectedWorkloadKeys((current) =>
					current.filter((key) => availableKeys.has(key)),
				);
				setDraftNotice(null);
			} else {
				const imported = new Set(importedDraft.workloadKeys);
				const matchedKeys = result.inventory.vms
				...
				if (matchedKeys.length === 0) {
					setDraftNotice(
						"The imported inventory draft did not match the refreshed platform inventory. Review the workload scope before continuing.",
					);
				} else if (matchedKeys.length < importedDraft.workloadKeys.length) {
					setDraftNotice(
						`${matchedKeys.length} of ${importedDraft.workloadKeys.length} imported workloads were matched after refresh. Review the workload scope before continuing.`,
					);
				} else {
					setDraftNotice(null);
				}
	...
	async function handleExecute() {
		...
		try {
			await executeMigration(
				migrationID,
				buildExecutionPayload(approvedBy, approvalTicket),
			);
