...
> = {
	draft: {
		...
		tone: "neutral",
		description:
			"The migration scope is being prepared and has not been validated for execution yet.",
	},
	ready: {
		label: "Ready",
		tone: "success",
		description:
			"The current plan can proceed based on the latest validation or saved plan state.",
	},
	warning: {
		label: "Warning",
		tone: "warning",
	...
	blocked: {
		...
		tone: "danger",
		description:
			"Execution is blocked until the current plan or environment issues are resolved.",
	},
	running: {
		label: "Running",
		tone: "accent",
		description:
			"Viaduct is actively executing or resuming migration phases for this run.",
	},
	failed: {
		label: "Failed",
		tone: "danger",
	...
	completed: {
		...
		tone: "success",
		description:
			"The migration finished successfully and the recorded run is complete.",
	},
	"rolled-back": {
		label: "Rolled back",
		tone: "neutral",
...
export function getMigrationWorkflowStatus(
	migration:
		| Pick<MigrationMeta, "phase">
		| Pick<MigrationState, "phase" | "pending_approval" | "errors">
		| null
		| undefined,
): MigrationWorkflowStatus {
	if (!migration) {
...
export function getPersistedMigrationListPresentation(
	status: PersistedMigrationListStatus,
): WorkflowStatusPresentation {
	switch (status) {
		case "planned":
			return {
				...
				tone: "neutral",
				description:
					"The migration plan is persisted, but list metadata alone does not expose readiness or approval details.",
			};
		case "running":
...
export function countPersistedMigrationListStatuses(
	migrations: MigrationMeta[],
): Record<PersistedMigrationListStatus, number> {
...
export function isMigrationExecutionActivePhase(
	phase: MigrationPhase,
): boolean {
