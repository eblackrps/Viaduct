import type { MigrationExecutionRequest, MigrationSpec, Platform, VirtualMachine } from "../../types";
      ...
      overrides: {
        ...
        network_map:
          Object.keys(networkMap).length > 0
            ? Object.fromEntries(Object.entries(networkMap).filter(([, value]) => value.trim() !== ""))
            : undefined,
      },
    })),
    options: {
      ...
      window:
        scheduledStart || scheduledEnd
          ? {
              not_before: scheduledStart ? new Date(scheduledStart).toISOString() : undefined,
              not_after: scheduledEnd ? new Date(scheduledEnd).toISOString() : undefined,
            }
          : undefined,
      approval: approvalRequired
        ? {
            ...
            approved_at: approvedBy.trim() && approvalRecordedAt ? approvalRecordedAt : undefined,
...
export function validateMigrationDraft(selectedCount: number, sourceAddress: string, targetAddress: string): string | null {
  if (selectedCount === 0) {
...
export function buildExecutionPayload(approvedBy: string, ticket: string): MigrationExecutionRequest | undefined {
  const payload = {
    approved_by: approvedBy.trim() || undefined,
...
export function resolveExecutionWindowState(start: string, end: string): ExecutionWindowState {
  const now = Date.now();
  ...
  if (!startMs && !endMs) {
    return { kind: "unset", label: "No execution window", summary: "No execution window" };
  }
  if (startMs && now < startMs) {
    return { kind: "not-started", label: startDate!.toLocaleString(), summary: "Window not open yet" };
  }
  if (endMs && now > endMs) {
    return { kind: "closed", label: endDate!.toLocaleString(), summary: "Execution window closed" };
  }
  return { kind: "open", label: (startDate ?? endDate)!.toLocaleString(), summary: "Execution window open" };
}
