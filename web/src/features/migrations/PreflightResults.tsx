  ...
  return (
    <div>
      ...
      <div className="mt-3 space-y-3">
        {checks.length > 0 ? (
          checks.map((check) => (
            <article key={check.name} className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-sm text-slate-600">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <p className="font-semibold text-ink">{getReadableCheckName(check.name)}</p>
                <StatusBadge tone={getCheckTone(check.status)}>{check.status}</StatusBadge>
              </div>
              <p className="mt-2 text-slate-500">{check.message}</p>
            </article>
          ))
        ) : <p className="rounded-2xl bg-slate-50 px-4 py-3 text-sm text-slate-600">{emptyMessage}</p>}
      </div>
    </div>
  );
}
